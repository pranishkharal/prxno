...
async def on_message(message):
    ...
    if session and message.attachments:
        ...
        is_png = (
            ...
            or attachment.content_type == "image/png"
        )
        ...
        if is_png:
            ...
            overlay_path = (
                OVERLAY_DIR
                / f"{message.author.id}_{attachment.filename}"
            )

            await attachment.save(overlay_path)

            session.overlay_file = overlay_path
            session.options["overlay"] = True

            await message.channel.send(
                "🖼️ **Overlay uploaded!**\n"
                "✅ It will automatically be placed at the **bottom center**.\n\n"
                f"Current settings: **{session.summary()}**"
            )
