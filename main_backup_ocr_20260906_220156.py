...
def get_overlay_file(size):
    """
    Automatically select the Kick overlay.

    Supports both WEBP and PNG.
    Format-specific overlay is preferred.
    Generic kick.webp / kick.png is used as fallback.
    """

    base_name = OVERLAY_FILES.get(size, "kick")

    # Prefer WEBP
    webp_file = OVERLAY_DIR / f"{base_name}.webp"

    if webp_file.exists():
        print(f"Using automatic WEBP overlay: {webp_file}")
        return webp_file

    # PNG fallback
    png_file = OVERLAY_DIR / f"{base_name}.png"

    if png_file.exists():
        print(f"Using automatic PNG overlay: {png_file}")
        return png_file

    # Generic WEBP fallback
    fallback_webp = OVERLAY_DIR / "kick.webp"

    if fallback_webp.exists():
        print(f"Format overlay not found. Using fallback: {fallback_webp}")
        return fallback_webp

    # Generic PNG fallback
    fallback_png = OVERLAY_DIR / "kick.png"

    if fallback_png.exists():
        print(f"Format overlay not found. Using fallback: {fallback_png}")
        return fallback_png

    print("WARNING: No Kick overlay found in overlays folder.")
    return None
